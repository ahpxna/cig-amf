...
from cig_amf_v2.influence_signature import (
    N_SEMANTIC_ROLES,
    ROLE_ANOMALOUS,
    ROLE_BENEFICIAL,
    ROLE_HARMFUL,
...
class PeripheralMultiMemoryV2(nn.Module):
    """
    ...
    def _normalise_inputs(self, periph_items) -> torch.Tensor:
        ...
        if x.shape[-1] != self.item_dim:
            raise ValueError(
                f"PeripheralMultiMemoryV2 expected item_dim={self.item_dim}, "
                f"got {x.shape[-1]}"
            )
...
PeripheralMultiMemory = PeripheralMultiMemoryV2
