...
try:
    ...
    print("ERROR: FinalCIGAMFRunner was not found in runners/final_runner.py")
    sys.exit(1)
...
try:
    ...
    print("ERROR: pair_specificity_score was not found in models/ego_conditioned_latent.py")
    sys.exit(1)
...
def run_step1():
    ...
    print("Initialising the environment and Final-CIGAMF model...")
    env = make_main_env(
    ...
    print("Running 60 episodes (10 warm-up + 50 learned-stage)...")
    runner.run(n_episodes=60, eval_every=10)
    
    print("\n" + "="*60)
    print(" STEP 1 RESULTS — FOUR V2 DIAGNOSTICS:")
    print("="*60)
    
    # 1. Ensemble Disagreement
    try:
        disagreement = runner.belief_modules[0].get_mean_uncertainty()
        print(f"[1] Ensemble Disagreement : {disagreement:.4f} \t(expected: > 0)")
    except Exception as e:
        print(f"[1] Ensemble Disagreement : [ERROR] {e}")

    # 2. Hit Max Rate
    try:
        hit_max_rate = runner.belief_modules[0].get_saturation_stats()["hit_max_rate"]
        print(f"[2] Hit Max Rate          : {hit_max_rate:.4f} \t(expected: < 1.0)")
    except Exception as e:
        print(f"[2] Hit Max Rate          : [ERROR] {e}")

    # 3. Usage Entropy Ratio
    try:
        entropy = runner.periph_module.get_slot_diagnostics()["usage_entropy_ratio"]
        print(f"[3] Usage Entropy Ratio   : {entropy:.4f} \t(expected: > 0.5)")
    except Exception as e:
        print(f"[3] Usage Entropy Ratio   : [ERROR] {e}")

    # 4. Specificity Ratio
    try:
        pair_rel = runner.pair_rel_module
        spec_ratio = pair_specificity_score(pair_rel, env.n_agents)["specificity_ratio"]
        print(f"[4] Specificity Ratio     : {spec_ratio:.4f} \t(expected: < 1.0)")
    except Exception as e:
        print(f"[4] Specificity Ratio     : [ERROR] {e}")
